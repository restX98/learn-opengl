...
const char* WINDOW_TITLE = "Lighting caster: Spotlight";
...
int main() {
  ...
  Shader shader(SHADERS_PATH "13_shader.vert", SHADERS_PATH "13_object_shader_spotlight.frag");
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    shader.setVec3("light.position", camera.Position); // Spotlight relative to the camera view
    shader.setVec3("light.direction", camera.Front);
    // We set the cutOff as cos of the angle because inside the shader we
    // calculate the light direction for each fragment as dot product between
    // the LightDir and the SpotDir vector and the dot product returns a
    // cosine value and not an angle.
    // In this way we prevent the cos calculation for each fragment and we do
    // it only for each frame.
